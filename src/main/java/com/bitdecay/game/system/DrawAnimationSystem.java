 ...
 */
public class DrawAnimationSystem extends AbstractDrawableSystem {

    public DrawAnimationSystem(AbstractRoom room) {
        super(room);
    }
        ...
        gobs.forEach(gob ->
            gob.forEachComponentDo(AnimationComponent.class, animation -> {
                gob.forEachComponentDo(PositionComponent.class, pos -> {
                    animation.update();
                    spriteBatch.draw(animation.texture, pos.x, pos.y);
                });
            })
        );
        spriteBatch.end();
